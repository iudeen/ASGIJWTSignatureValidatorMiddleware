...
async def test_protected_request_success() -> None:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        r = await client.post("/", content=encoded_body, headers={"content-type": "application/json"})
        assert r.status_code == 200
        assert r.json() == body
...
async def test_protected_request_tampered() -> None:
    ...
    with pytest.raises(HTTPException):
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            body = {"test": "test"}
            encoded_body = encode_payload_modify(body, "hello" * 100, "HS256")
            await client.post("/", content=encoded_body, headers={"content-type": "application/json"})
...
async def test_unprotected_request_large() -> None:
    ...
    app = EncodedPayloadSignatureMiddleware(
        app, jwt_secret="hello" * 100, jwt_algorithms=["HS256"], protect_hosts=["example.com"]
    )
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        body = await generate_large_data()
